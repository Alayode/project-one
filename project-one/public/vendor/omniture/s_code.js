/* SiteCatalyst code version: H.24.1v3 02142014
Copyright 1996-2011 Adobe, Inc. All Rights Reserved
More info available at http://www.omniture.com */

var reporting_account = "cablevis-onet-prod"; 
var host = window.location.hostname;
if(host.match(/espanol/)){
  reporting_account = "cablevis-onet-prod-esp";
}else if(host.match(/stg/)){
  reporting_account = "cablevis-stage3";
}else if(host.match(/stage/)){
  reporting_account = "cablevis-stage3";
}else if(host.match(/preview.preprod/)){
  reporting_account = "cablevis-stage3";  
}else if(host.match(/preprod/)){
  reporting_account = "cablevis-stage3";  
}else if(host.match(/dev/)){
  reporting_account = "cablevis-dev3";
}
var s_account=reporting_account;

/*Use multi-suite tagging to send to multiple report suites
Example: s_account="cablevisionodn,cablevisionodnwest";
This will send the hit to the global suite as well as the west suite*/
...
/*Place whatever is considered "internal" in s.linkInternalFilters.
Example: s.linkInternalFilters="javascript:,optimum.net,optimum.com,west.optimum.com";
This internal filters list should match the internal filters specified in the SiteCatalyst admin console.*/
//s.linkInternalFilters="javascript:,optimum.net,stg.optimum.net, stage.optimum.net, preview.preprod.optimum.net, preprod.optimum.net, voice.optimum.net, uwc.webmail.optimum.net, webtop.webmail.optimum.net, optimum.custhelp.com";
if(s_account === "cablevis-onet-prod-esp"){
	s.linkInternalFilters="javascript:,https://espanol.optimum.net,http://espanol.optimum.net";
}else{
	s.linkInternalFilters="javascript:,http://www.optimum.net,https://www.optimum.net,stg.optimum.net,stage.optimum.net,preview.preprod.optimum.net,preprod.optimum.net,voice.optimum.net,uwc.webmail.optimum.net,webtop.webmail.optimum.net,optimum.custhelp.com,http://optimum.custhelp.com";
}
...
function s_doPlugins(s) {
  ...
  /*Visitor Loyalty/Return Frequency Variables*/
  ...
  //percent page view
  if (s.eVar1) s.prop19 = s.getPreviousValue(s.eVar1,'s_ppn');
  var ppv = s.getPercentPageViewed(s.eVar1);
  if(ppv && typeof ppv === 'object' && ppv[0] === s.prop19 ){
    s.prop18 = ppv[1] + '|' + ppv[2];
  }
  
  /*Time Parting*/  
  s.prop20=s.getTimeParting('h','-5');
  s.prop26=s.getTimeParting('d','-5');
  s.prop27=s.getTimeParting('w','-5');
  if(s.prop20) s.eVar17= s.prop20 + ' - ' + s.prop26;
  
  /*User Account Actions*/
  if(s.prop29) s.eVar29="D=c29";
  if(s.prop30) s.eVar30="D=c30";
  if(s.prop35) s.eVar40="D=c35";   
  //s.eVar27=s.prop47="D=pageURL";
  s.eVar27="D=pageURL";
  var getCleanedPath = function(){
  ...
  var cleanUrl = window.location.hostname;  
  if(getCleanedPath() !== "") cleanUrl += '/' + getCleanedPath();  
  s.prop47=cleanUrl;
  
  /*Cross Visit Participation*/
  s.eVar57=s.crossVisitParticipation(s.campaign,'s_ev57','30','10','|','purchase','1');
  var SegmentStack=s.crossVisitParticipation(s.eVar41,'s_ev22','90','10',' > ','',1); 
  if(s.eVar41) {
    s.eVar22 = SegmentStack;
  }
  
  var PageStack=s.crossVisitParticipation(s.eVar1,'s_pgNAME1','90','10',' > ','',0); 
  if(s.eVar1) {s.prop1 = PageStack;}
  ...
  if(s.eVar46) {s.eVar46 += ' | '  + document.documentElement.clientWidth + 'x' + document.documentElement.clientHeight;}
  //Screen Size - 020714 
  //s.eVar47 = document.documentElement.clientWidth + 'x' + document.documentElement.clientHeight;
 
  //Rendered Experiences (with specific break points) - 020714
  //s.eVar48 = getRendered(); 
  ...
  s.eVar58=s.prop58="D=User-Agent";
  
  /*flash version*/
  s.detectRIA('s_ria','prop31','prop32','','','');  
  
  /*Set Up Link Tracking*/
  s.hbx_lt = "auto" // manual, auto
  s.setupLinkTrack("prop43,prop44,prop45,prop46","SC_LINKS");
  
  s.campaign=s.getValOnce(s.getQueryParam("s_cid"),"s_campaign_cid",0);
}
...
//Internal Campaign
if (!s.eVar44) s.eVar44 = s.getValOnce(s.getQueryParam('vcid,v_cid', '_').toLowerCase(), 's_v44', 0);
if (!s.eVar18) s.eVar18 = s.getValOnce(s.getQueryParam('icid,i_cid', '_').toLowerCase(), 's_v18', 0);

/* External Campaign
 * s_cid: Current Campaign URL Parameter
 */
if (!s.campaign) s.campaign = s.getValOnce(s.getQueryParam('scid,s_cid', '-_-').toLowerCase(), 's_campaign', 30);
if (s.campaign) {
    var ecidlinkParts = s.campaign.split('-_-');
    if (ecidlinkParts[0]) s.eVar26 = ecidlinkParts[0]; //External Campaign - Tactic
    if (ecidlinkParts[1]) s.eVar42 = ecidlinkParts[1]; //External Campaign - Campaign
    if (ecidlinkParts[2]) s.eVar43 = ecidlinkParts[2]; //External Campaign - Wildcard
}

//track custom links - ar version 01132014
...
/*get rendered based on width*/
function getRendered(){
	var dtype = 'Unknown';
	var dw = document.documentElement.clientWidth;
	if(dw >= 320 && dw <= 568) {
	  dtype = "Smartphone"
	}else if(dw >= 600 && dw <= 800) {
	  dtype = "Small Tablet";
	}else if(dw >= 768 && dw <= 1024) {
	  dtype = "Tablet";
	}else if(dw >= 1280 && dw <= 800) {
	  dtype = "Widescreen TV | Laptop";
	}else if (dw >= 1280) {
	  dtype = "Desktop | Large TV";
	}else{
	  dtype = "Unknown";
	}
	return dtype;
...
s.visitorNamespace = "optimum";
s.trackingServer="metrics.optimum.net";
s.trackingServerSecure="smetrics.optimum.net";
